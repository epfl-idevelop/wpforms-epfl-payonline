 ...
 * @since 1.0.0
 */
class WPForms_EPFL_LDAP extends WPForms_Field {
	...
	public function init() {
		...
		$this->name  = esc_html__( 'EPFL People', 'wpforms-epfl-payonline' );
		$this->type  = 'epfl-ldap';
		$this->group = 'EPFL';
		$this->icon  = 'fa-user';
		$this->order = 30;
		
		// Add this field into correspongin WPForms functions
		add_filter( 'wpforms_load_fields', array( $this, 'add_EPFL_LDAP_fields' ), 10, 1 );
		add_filter( 'wpforms_builder_fields_buttons', array( $this, 'add_EPFL_LDAP_fields_button' ), 100, 1 );

		// Define additional field properties.
		add_filter( 'wpforms_field_properties_epfl-ldap', array( $this, 'field_properties' ), 5, 3 );
		add_action( 'wpforms_frontend_js', array( $this, 'frontend_js' ) );
		// 
		//wp_enqueue_script( 'epfl-people', WPFORMS_EPFL_PAYONLINE_PLUGIN_URL . "js/epfl-people.js", array( 'jquery' ), WPFORMS_EPFL_PAYONLINE_VERSION, true );
		//wp_localize_script( 'epfl-people', 'ajaxurl', admin_url( 'admin-ajax.php' ));
		// 
		add_action( 'wp_enqueue_scripts', array( $this, 'enqueue_epfl_people' ));
		// add_action( 'admin_enqueue_scripts', array( $this, 'actions_test' ));

		add_action( 'wp_ajax_my_action', array( $this, 'my_action' ));
		add_action( 'wp_ajax_nopriv_my_action', array( $this, 'my_action' ));
	}
	
	// public function actions_test() {
	// 	add_action( 'wp_ajax_my_action', array( $this, 'my_action' ));
	// 	add_action( 'wp_ajax_nopriv_my_action', array( $this, 'my_action' ));
	// }
	// 
	function enqueue_epfl_people() {
		wp_enqueue_script( 'epfl-people', WPFORMS_EPFL_PAYONLINE_PLUGIN_URL . "js/epfl-people.js", array( 'jquery' ), WPFORMS_EPFL_PAYONLINE_VERSION, true );
		wp_localize_script( 'epfl-people', 'ajaxurl', admin_url( 'admin-ajax.php' ));
	}

	function add_EPFL_LDAP_fields ($fields) {
		// error_log("fields_btn--------------------------------------------");
		// error_log(var_export($fields, true));
		array_push($fields, $this->name);
		return $fields;
	}

	function add_EPFL_LDAP_fields_button ($fields) {
		$fields['EPFL']['group_name'] = esc_html__( 'EPFL', 'wpforms-epfl-payonline' );
		return $fields;
	}
 ...
 function test($entry/*, $form*/) {
	...
	public function field_display( $field, $deprecated, $form_data ) {
		...
		$primary['class'][] = 'wpforms-epfl-payonline-people';
		// Primary field.
		printf(
			'<input type="text" %s %s data-epfl-people="true"><div id="epfl-people-response"></div><a href="#" id="epfl-people-check">check</a>',
	...
	public function frontend_js( $forms ) {
		...
		// 
		// error_log("------------------------------");
		// error_log('WPFORMS_EPFL_PAYONLINE_PLUGIN_URL');
		// error_log(var_export(WPFORMS_EPFL_PAYONLINE_PLUGIN_URL));
		// error_log('get_template_directory_uri');
		// error_log(var_export(get_template_directory_uri()));
		// error_log("------------------------------");
		...
		// Either here or in the init() function
		// wp_enqueue_script( 'epfl-people', WPFORMS_EPFL_PAYONLINE_PLUGIN_URL . "js/epfl-people.js", array( 'jquery' ), WPFORMS_EPFL_PAYONLINE_VERSION, true );
		// wp_localize_script( 'epfl-people', 'ajaxurl', admin_url( 'admin-ajax.php' ));
	...
	private function validateEmail($wat) {
		preg_match("/[a-zA-Z0-9_.+-]+@[a-zA-Z0-9.-]+/", $wat, $matched, PREG_UNMATCHED_AS_NULL);
		return ($matched == null) ? false : $matched[0];
	}

	private function validateSciper($wat) {
		preg_match("/[A-Z]?\d+/", $wat, $matched, PREG_UNMATCHED_AS_NULL);
		return ($matched == null) ? false : $matched[0];
	}

	private function validateName($wat) {
		preg_match("/[ [:alnum:]]+/ui", $wat, $matched, PREG_UNMATCHED_AS_NULL);
		return ($matched == null) ? false : $matched[0];
	}
	...
	public function my_action() {
		$searchNeedle = trim($_POST['searchNeedle']);
		require(__DIR__ . '/lib/EPFLLDAP.php');

		$ldap = new EPFLLDAP();

		if ($email = $this->validateEmail($searchNeedle)) {
			error_log('Lookup LDAP email:' . var_export($email, true));
			$entries = $ldap->searchEmail($email);

		} elseif ($sciper = $this->validateSciper($searchNeedle)) {
			error_log('Lookup LDAP sciper:' . var_export($sciper, true));
			$entries = $ldap->searchSciper($sciper);

		} elseif ($name = $this->validateName($searchNeedle)) {
			error_log('Lookup LDAP name:' . var_export($name, true));
			$entries = $ldap->searchName($name);
		...
		//error_log(var_export(WPFORMS_EPFL_PAYONLINE_PLUGIN_URL));
error_log('LDAP RETURNED ENTRIES:' . var_export($entries, true));

		$data = $entries[0];
		// URF-8 encode LDAP entries 
		...
		$response = array(
			data => $data,
			//accred => EPFLWSAccred::getBySciper($data["sciper"]),
			status => "success"
		);
		
		$response_json = json_encode($response);
		
		if ($json_error_code = json_last_error()) {
			$response_json = json_encode(array(
				status => "json_encode_error",
				json_encode_error_code => $json_error_code
			));
		}
			
		header('Content-Type: application/json');
		echo $response_json;
		error_log('LDAP RETURNED ENTRIES:' . var_export($data, true));
		die();
	}
	/**
